...
const incomeCategories = ["বেতন", "ফ্রিল্যান্স", "উপহার", "বিনিয়োগ", "ব্যবসা", "ভাড়া", "অন্যান্য"];
const expenseCategories = ["খাবার", "যানবাহন", "বিল", "বিনোদন", "স্বাস্থ্য", "শিক্ষা", "কেনাকাটা", "অন্যান্য"];

const categoryIcons: Record<string, string> = {
  বেতন: "💼", ফ্রিল্যান্স: "💻", উপহার: "🎁", বিনিয়োগ: "📈", ব্যবসা: "🏢", ভাড়া: "🏠",
  খাবার: "🍽️", যানবাহন: "🚗", বিল: "📄", বিনোদন: "🎬", স্বাস্থ্য: "🏥", শিক্ষা: "📚", কেনাকাটা: "🛍️", অন্যান্য: "📌",
};
...
export default function AddTransaction({ defaultType, transaction, onSave, onCancel }: Props) {
  ...
  return (
    <div className="flex flex-col min-h-full">
      {/* Header */}
      <div className={`${isIncome ? "bg-emerald-600" : "bg-rose-500"} px-5 pt-12 pb-8 rounded-b-3xl transition-colors duration-300`}>
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-white text-lg font-semibold">{transaction ? "লেনদেন সম্পাদনা করুন" : "লেনদেন যোগ করুন"}</h1>
        </div>
        ...
        <div className="flex bg-white/20 rounded-2xl p-1 mb-5">
          <button
            ...
            className={`flex-1 py-2.5 rounded-xl text-sm font-semibold transition-all ${type === "income" ? "bg-white text-emerald-600 shadow" : "text-white"}`}
          >
            আয়
          </button>
          <button
            onClick={() => { setType("expense"); setCategory(""); }}
            className={`flex-1 py-2.5 rounded-xl text-sm font-semibold transition-all ${type === "expense" ? "bg-white text-rose-500 shadow" : "text-white"}`}
          >
            ব্যয়
          </button>
        </div>
      ...
      <div className="flex-1 px-5 pt-5 flex flex-col gap-4">
        ...
        <button
          ...
          className={`w-full py-4 rounded-2xl text-white font-semibold text-base mt-2 transition-all active:scale-95 disabled:opacity-40 ${
            isIncome ? "bg-emerald-600 shadow-lg shadow-emerald-200" : "bg-rose-500 shadow-lg shadow-rose-200"
          }`}
        >
          সংরক্ষণ করুন
        </button>
